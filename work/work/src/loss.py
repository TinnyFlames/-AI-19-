...
class StyleLoss(nn.Layer):
    ...
    def __call__(self, x, y, mask=None):
        ...
        x_vgg, y_vgg = x, y
        relu2_size = x_vgg['relu2_2'].shape[-1]
        relu3_size = x_vgg['relu3_4'].shape[-1]
        relu4_size = x_vgg['relu4_4'].shape[-1]
        relu5_size = x_vgg['relu5_2'].shape[-1]
        # Compute loss
        style_loss = 0.0
        if type(mask) != type(None):
            mask1 = F.interpolate(mask, size=(relu2_size,relu2_size), mode='nearest')
            mask2 = F.interpolate(mask, size=(relu3_size,relu3_size), mode='nearest')
            mask3 = F.interpolate(mask, size=(relu4_size,relu4_size), mode='nearest')
            mask4 = F.interpolate(mask, size=(relu5_size,relu5_size), mode='nearest')
        else:
            mask1 = 1
            mask2 = 1
            mask3 = 1
...
class PerceptualLoss(nn.Layer):
    ...
    def __call__(self, x, y, mask):
        ...
        # x_vgg, y_vgg = self.vgg(x), self.vgg(y)
        x_vgg, y_vgg = x, y
        relu2_size = x_vgg['relu2_1'].shape[-1]
        relu3_size = x_vgg['relu3_1'].shape[-1]
        relu4_size = x_vgg['relu4_1'].shape[-1]
        relu5_size = x_vgg['relu5_1'].shape[-1]
        content_loss = 0.0
        if type(mask) != type(None):
            mask1 = mask
            mask2 = F.interpolate(mask, size=(relu2_size,relu2_size), mode='nearest')
            mask3 = F.interpolate(mask, size=(relu3_size,relu3_size), mode='nearest')
            mask4 = F.interpolate(mask, size=(relu4_size,relu4_size), mode='nearest')
            mask5 = F.interpolate(mask, size=(relu5_size,relu5_size), mode='nearest')
        else:
            mask1 = 1
            mask2 = 1
            mask3 = 1
